...
async function serverGet(ep: string): Promise<any> {
  const r = await Bun.$`curl -s --max-time 5 ${SERVER}${ep}`.text()
  return JSON.parse(r.trim())
}

async function serverPost(ep: string, data: any): Promise<any> {
  const r = await Bun.$`curl -s --max-time 5 -X POST -H "Content-Type: application/json" -d ${JSON.stringify(data)} ${SERVER}${ep}`.text()
  return JSON.parse(r.trim())
}

async function runSendCmd(context: any, args: string[]): Promise<string> {
  const script = path.join(context.worktree, "resources/tools/send_cmd.py")
  const proc = Bun.$`python ${script} ${args}`.cwd(context.worktree)
...
export const agent_sessions = tool({
  ...
  args: {},
  async execute() {
    const data = await serverGet("/status")
...
export const agent_cmd = tool({
  ...
  async execute(input) {
    ...
    const r = await Bun.$`python ${script} ${cmdArgs}`.text()
    return r.trim()
  },
})
...
export const agent_navigate = tool({
  ...
  async execute(input) {
    const script = path.join(".", "resources/tools/send_cmd.py")
    const r = await Bun.$`python ${script} ${input.session} navigate ${input.url}`.text()
    return r.trim()
  },
})
...
export const agent_click = tool({
  ...
  async execute(input) {
    const script = path.join(".", "resources/tools/send_cmd.py")
    const r = await Bun.$`python ${script} ${input.session} click ${String(input.index)}`.text()
    return r.trim()
  },
})
...
export const agent_type = tool({
  ...
  async execute(input) {
    const script = path.join(".", "resources/tools/send_cmd.py")
    const r = await Bun.$`python ${script} ${input.session} type ${String(input.index)} ${input.text}`.text()
    return r.trim()
  },
})
...
export const agent_ping = tool({
  ...
  async execute(input) {
    const script = path.join(".", "resources/tools/send_cmd.py")
    const r = await Bun.$`python ${script} ${input.session} ping`.text()
    return r.trim()
  },
})
