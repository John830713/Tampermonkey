...
(function() {
    ...
    function loadAgentCore() {
        ...
        fetchText(url, function(err, code) {
            if (err) {
                console.error('[Loader] failed to load agent.core.js:', err);
                return;
            }
            executeScript(code);
            console.log('[Loader] agent.core.js loaded');
        });
    }
    ...
    function loadModules() {
        ...
        fetchJSON(url, function(err, modules) {
            if (err) {
                console.error('[Loader] failed to load modules:', err);
                return;
            }
            ...
            modules.forEach(function(m) {
                ...
                if (!isModuleEnabled(m, overrides)) {
                    console.log('[Loader] module disabled: ' + m.name);
                    return;
                }
                ...
                fetchText(scriptUrl, function(err, code) {
                    if (err) {
                        console.error('[Loader] failed to load ' + m.name + ':', err);
                        return;
                    }
                    executeScript(code);
                    console.log('[Loader] module loaded: ' + m.name);
                });
            });

            if (matchedModules.length > 0) {
                console.log('[Loader] ' + loadedCount + '/' + matchedModules.length + ' module(s) loaded for ' + locationHref);
            }
    ...
    function buildTogglePanel(matchedModules, overrides) {
        ...
        GM_addStyle(
            ...
            '  border: 2px solid #666; background: #444; color: #fff;' +
            '  font-size: 13px; cursor: pointer;' +
            '  display: flex; align-items: center; justify-content: center;' +
            '  box-shadow: 0 1px 4px rgba(0,0,0,0.3); transition: all 0.15s;' +
            '  line-height: 1; padding: 0; font-family: inherit;' +
            '}' +
            '#a1-toggle-btn:hover { transform: scale(1.1); }' +
            '#a1-mod-panel {' +
            '  display: none; position: fixed; bottom: 48px; left: 12px; z-index: 2147483647;' +
            ...
            '#a1-mod-panel .mod-title { font-size: 11px; color: #888; margin-bottom: 8px; }' +
            ...
            '#a1-mod-panel .mod-label { font-size: 13px; color: #333; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }' +
            '#a1-mod-panel .mod-sub { font-size: 10px; color: #999; margin-top: 1px; }' +
            '#a1-mod-panel .mod-switch { position: relative; width: 36px; height: 20px; cursor: pointer; flex-shrink: 0; margin-left: 8px; }' +
            '#a1-mod-panel .mod-switch input { display: none; }' +
            '#a1-mod-panel .mod-slider { position: absolute; inset: 0; background: #ccc; border-radius: 20px; transition: background 0.2s; }' +
            '#a1-mod-panel .mod-slider::before { content: ""; position: absolute; width: 16px; height: 16px; left: 2px; top: 2px; background: #fff; border-radius: 50%; transition: transform 0.2s; }' +
            '#a1-mod-panel .mod-switch input:checked + .mod-slider { background: #2e7d32; }' +
            '#a1-mod-panel .mod-switch input:checked + .mod-slider::before { transform: translateX(16px); }'
        );
        ...
        fetchJSON(SERVER + '/modules?t=' + Date.now(), function(err, data) {
            ...
            try {
                ...
                if (currentHash !== 0 && newHash !== currentHash) {
                    console.log('[Loader] modules changed, reloading...');
                    location.reload();
                }
                currentHash = newHash;
            } catch(e) {}
