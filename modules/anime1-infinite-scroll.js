...
// @version      2.0
// @description  動畫列表無限滾動 + 折疊卡片 + 跳頁器 + 分類頁下載面板 + 單集自動下載
// @author       You
// @match        *://anime1.me/*
// @match        *://*.v.anime1.me/*
// @grant        GM_addStyle
...
(function() {
    ...
    (function initCategoryPage() {
        ...
        if (!/anime1\.me\/\?cat=\d+/.test(location.href)) return;
        ...
        GM_addStyle(`
            ...
            #a1-dl-panel .dl-title { color: #e67e22; font-weight: bold; }
            #a1-dl-panel .dl-toggle { cursor: pointer; font-size: 12px; color: #aaa; }
            #a1-dl-panel .dl-toggle:hover { color: #fff; }
            #a1-dl-panel .dl-batch {
                width: 100%; background: #ff4757; color: white; border: none;
                padding: 10px; border-radius: 6px; cursor: pointer;
                font-weight: bold; margin-bottom: 12px;
            }
            #a1-dl-panel .dl-batch:hover { background: #e63946; }
            #a1-dl-panel .dl-grid {
                display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px;
            }
            #a1-dl-panel .dl-ep {
                background: #1e90ff; color: white; border: none;
                padding: 6px 2px; border-radius: 4px; cursor: pointer; font-size: 11px;
            }
            #a1-dl-panel .dl-ep:hover { background: #187bcd; }
        `);

        const panel = document.createElement('div');
        panel.id = 'a1-dl-panel';
        panel.innerHTML = `
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
                <span class="dl-title">快速下載 (${episodes.length} 集)</span>
                <span class="dl-toggle">[收合]</span>
            </div>
            <div class="dl-content">
                <button class="dl-batch">一鍵批次全集下載</button>
        ...
        episodes.forEach((ep, idx) => {
            const label = ep.title.match(/\[(.*?)\]/)?.[1] || (idx + 1);
            const btn = document.createElement('button');
            btn.className = 'dl-ep';
            btn.textContent = label;
            btn.title = ep.title;
            btn.addEventListener('click', () => GM_openInTab(ep.url, { active: false, insert: true }));
            grid.appendChild(btn);
        });

        const content = panel.querySelector('.dl-content');
        panel.querySelector('.dl-toggle').addEventListener('click', function() {
            const hidden = content.style.display === 'none';
            content.style.display = hidden ? '' : 'none';
            this.textContent = hidden ? '[收合]' : '[展開]';
            panel.style.width = hidden ? '300px' : '120px';
        });

        panel.querySelector('.dl-batch').addEventListener('click', function() {
            if (!confirm(`確定依序下載共 ${episodes.length} 集？`)) return;
            this.disabled = true;
            episodes.forEach((ep, i) => {
                setTimeout(() => {
                    GM_openInTab(ep.url, { active: false, insert: true });
                    this.textContent = `處理中 (${i + 1}/${episodes.length})`;
                    if (i === episodes.length - 1) {
                        this.disabled = false;
                        this.textContent = '一鍵批次全集下載';
                    }
                }, i * 3000);
            });
        });
